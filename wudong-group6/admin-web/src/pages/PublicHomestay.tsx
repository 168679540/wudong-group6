    ...
    <Layout>
      ...
      <Modal open={!!detail} onCancel={() => { setDetail(null); setReviews([]); }} footer={[<Button key="back" onClick={() => { setDetail(null); setReviews([]); }}>返回</Button>, <Button key="book" type="primary" onClick={() => { const d = detail; setDetail(null); setTimeout(() => d && openBook(d), 100); }}>预订房间 ¥{detail?.pricePerNight}/晚</Button>]} width={720} title={detail?.name}>
        {detail && (<div><img src={detail.coverImage} alt={detail.name} style={{ width: '100%', maxHeight: 380, objectFit: 'cover', borderRadius: 8, marginBottom: 16 }} onError={e => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/600x400'; }} />
          <Descriptions column={2} bordered size="small"><Descriptions.Item label="价格"><span style={{ color: '#f5222d', fontSize: 20, fontWeight: 'bold' }}>¥{detail.pricePerNight}</span> /晚</Descriptions.Item><Descriptions.Item label="房间数">{detail.roomCount} 间</Descriptions.Item><Descriptions.Item label="地址" span={2}><EnvironmentOutlined /> {detail.address}</Descriptions.Item><Descriptions.Item label="设施" span={2}>{detail.amenities?.split(',').map((a: string) => <Tag key={a} color="green"><CheckCircleOutlined /> {a.trim()}</Tag>)}</Descriptions.Item><Descriptions.Item label="入住时间">{(detail as any).checkInTime || '14:00'}</Descriptions.Item><Descriptions.Item label="离店时间">{(detail as any).checkOutTime || '12:00'}</Descriptions.Item><Descriptions.Item label="取消政策" span={2}><span style={{color:'#fa8c16'}}>{(detail as any).cancellationPolicy || '入住前3天免费取消，1-3天扣30%，当天不退'}</span></Descriptions.Item><Descriptions.Item label="介绍" span={2}>{detail.description}</Descriptions.Item></Descriptions>
          {/* 评价 */}
