...
const Dashboard: React.FC = () => {
  ...
  const moduleGMVOption = useMemo(() => ({
    ...
    yAxis: { type: 'value' as const, name: '元' },
    series: [{ type: 'bar' as const, data: moduleGMV.map(m => m.value), barWidth: '50%', label: { show: true, position: 'top' as const, formatter: '¥{c}', fontSize: 10 }, itemStyle: { borderRadius: [4, 4, 0, 0], color: (p: any) => MODULE_COLORS[moduleGMV[p.dataIndex]?.name] || '#d4a14b' } }],
  }), [moduleGMV]);
