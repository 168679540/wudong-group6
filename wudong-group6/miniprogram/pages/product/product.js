...
Page({ data: { list: [], cats: ['全部'], cat: '', minPrice: '', maxPrice: '', detail: null, buyTarget: null, specQty: 1, specSizes: [], specColors: [], specSizeIdx: 0, specColorIdx: 0 },
  ...
  doBuy() { var p = this.data.buyTarget; if (!p) return;
    ...
    var specDesc = [size, color].filter(Boolean).join(' ');
    var name = p.name; if (specDesc) name += ' ' + specDesc; name += ' ×' + this.data.specQty;
    api.createOrder({ type: '商品', amount: Number(p.price) * this.data.specQty, merchantId: p.merchantId, itemName: name, itemImage: p.coverImage }).then(r2 => { if (r2.success) { wx.showToast({ title: '购买成功！' }); this.setData({ buyTarget: null }); } else { wx.showToast({ title: r2.message || '下单失败', icon: 'none' }); } }).catch(() => wx.showToast({ title: '下单失败', icon: 'none' }));
  }
});
